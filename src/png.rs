...
impl Display for PngError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PngError::MissingChunk => write!(f, "Provided chunk is missing from the PNG"),
            PngError::CorruptedData => write!(f, "The bytes do not constitute a valid PNG")
        }
    }
}
...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        if let Some((i, _)) = self.chunks.iter().enumerate().find(|(index, chunk)| chunk.chunk_type().to_string() == chunk_type) {
            Ok(self.chunks.remove(i))
        } else {
            Err(Box::new(PngError::MissingChunk))
        }
    }
    ...
    fn as_bytes(&self) -> Vec<u8> {
        Self::STANDARD_HEADER.into_iter().chain(
            self.chunks.iter().flat_map(|chunk| chunk.as_bytes()))
                .collect()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        if value.len() < Png::STANDARD_HEADER.len() {
            return Err(Box::new(PngError::CorruptedData));
        }
        ...
        let mut chunks_bytes = &value[Png::STANDARD_HEADER.len()..];
        let mut chunks: Vec<Chunk> = vec![];
        loop {
            if chunks_bytes.len() < 4 {
                return Err(Box::new(PngError::CorruptedData));
            }

            let length = u32::from_be_bytes(chunks_bytes[..4].try_into().unwrap());

            if chunks_bytes.len() < (length + 12) as usize {
                return Err(Box::new(PngError::CorruptedData));
            }

            let new_chunk = Chunk::try_from(&chunks_bytes[..(length + 12) as usize]);

            if let Err(_) = new_chunk {
                return Err(Box::new(PngError::CorruptedData));
            }

            chunks.push(new_chunk.unwrap());
            chunks_bytes = &chunks_bytes[(length + 12) as usize..];

            if chunks_bytes.len() == 0 {
                break;
            }
        }
